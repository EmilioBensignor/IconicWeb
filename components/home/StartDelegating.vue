<template>
  <section
    class="w-full startDelegating columnAlignCenter gap-4 px-3 py-5"
    ref="delegatingSection">
    ...
    <div class="delegatingContainerDesktop" ref="desktopContainer">
    ...
    <NuxtLink :to="routes.CONTACT_US" class="primaryButton"> Book a call </NuxtLink>
  </section>
</template>
    ...
    methods: {
      ...
      setupIntersectionObserver() {
        if ("IntersectionObserver" in window) {
          ...
          this.$nextTick(() => {
            this.observer.observe(this.$refs.delegatingSection);
          });
        }
      },
      startAutoChange() {
        if (!this.autoChangeInterval) {
          this.autoChangeInterval = setInterval(() => {
            if (this.isVisible && window.innerWidth >= 850) {
